...
def main():
    ...
    for t in range(args.max_iterations):
        ...
        # send out our model uk and pk
        dist.send(tensor=uk, dst=0, tag=1)
        dist.send(tensor=pk, dst=0, tag=2)
        print("Model sent to master (rank 0)")

        # receive master model
        dist.recv(tensor=u0, src=0, tag=0)
        print("Model received from master (rank 0)")
